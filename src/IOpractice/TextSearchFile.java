...
public class TextSearchFile {
    ...
    public static File[] searchFile(File folder, final String keyWord) {// 递归查找包含关键字的文件
        ...
        File[] subFolders = folder.listFiles(new FileFilter() {// 运用内部匿名类获得文件
            ...
            public boolean accept(File pathname) {// 实现FileFilter类的accept方法
                if (pathname.isFile())// 如果是文件
                    ...
                    // 如果是目录
                    countFolders++;
                if (pathname.isDirectory() || (pathname.isFile() && pathname.getName().endsWith(keyWord)))// 目录或文件包含关键字
                    return true;
                return false;
            }
        });
        ...
        for (int i = 0; i < subFolders.length; i++) {// 循环显示文件夹或文件
            ...
            } else if(subFolders[i].isDirectory()){// 如果是文件夹，则递归调用本方法，然后把所有的文件加到结果列表中
    ...
    public static void main(String[] args) {// java程序的主入口处
        File folder = new File("F:/onetwo");// 默认目录
        String keyword = "java";
        ...
        System.out.println("在 " + folder + " 以及所有子文件时查找文件包含关键字" + keyword+"的文件");
